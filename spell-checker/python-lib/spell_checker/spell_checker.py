...
class SpellChecker:
    ...
    def _fix_typos_in_word(self, word, sym_spell):
        ...
        """
        correction = sym_spell.lookup(word, 
    ...
    def _fix_typos_in_document(self, string_or_token_list, sym_spell):
        ...
        if not self.params['word_segmentation']:
            ...
            try:
                ...
                return sym_spell.word_segmentation(string_or_token_list,
                                                   ignore_token=self.params['ignore_token']).corrected_string
            except:
    ...
    def compute(self, df):
        ...
        for col_txt, col_spellchecked in self.col_txt_spellchecked_dict.items():
            ...
            df[col_spellchecked] = df.apply(lambda x:self._fix_typos_in_document(x[col_preprocessed], 
                                                                                 self.sym_spells[x[col_lang]]), axis=1)
