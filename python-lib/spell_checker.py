...
class SpellChecker:
    ...
    DISTANCE = 2 # distance max under which a correction is returned
    SUGGESTION_VERBOSITY = Verbosity.TOP # returns only the closest word
    TRANSFERT_CASING = False # the original casing (lowercase and uppercase) is not carried over the text. Symspellpy returns errors for some words. Note that if a word is all uppercase, symspellpy returns the lowercased word. 
    ...
    def __init__(self, 
        ...
        if self.language_selection == 'from_column':
            self.language_column = language_column
        if self.language_selection == 'from_list':
    ...
    def _add_sym_spell_objects(self, new_lang_code_list: List):
        ...
        for lang_code in new_lang_code_list:
            ...
            self.sym_spells[lang_code] = SymSpell(max_dictionary_edit_distance=2)
    ...
    def _fix_typos_in_word(self, word: AnyStr, lang: AnyStr) -> (AnyStr, List, int):
        ...
        correction = self.sym_spells[lang].lookup(word, 
                                                  ...
                                                  max_edit_distance=self.DISTANCE, 
                                                  transfer_casing=self.TRANSFERT_CASING,
    ...
    def compute(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for txt_col in self.text_column_list:        
            
            # Name creation of new columns
            preprocess_col = generate_unique(txt_col, existing_column_names, 'preprocess')
            existing_column_names.append(preprocess_col)
            
            spellcheck_col = generate_unique(txt_col, existing_column_names, 'spell_check')
            existing_column_names.append(spellcheck_col)
                
            misspell_col = generate_unique(txt_col, existing_column_names, 'misspell')
            existing_column_names.append(misspell_col)

            misspell_count_col = generate_unique(txt_col, existing_column_names, 'misspell_count')
            existing_column_names.append(misspell_count_col)

            # column containing a tuple (corrected doc, misspells, count)
            spellcheck_output_temp = generate_unique(txt_col, existing_column_names, 'spellcheck_output_temp')
